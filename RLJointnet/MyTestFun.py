...
import argparse
import os
from data_utils.MyDataLoader import TestDataLoad
...
def parse_args():
    ...
    parser.add_argument('--log_dir', type=str, default='2025-06-06_06-06', help='experiment root')
...
def main(args):
    ...
    TEST_DATASET_WHOLE_SCENE = TestDataLoad(root,  block_points=NUM_POINT)
    ...
    with torch.no_grad():
        ...
        for batch_idx in range(num_batches):
            ...
            # 保存预测的标签到点云数据的最后一列
            final_data = np.hstack((whole_scene_data, pred_label.reshape(-1, 1)))

            filename = os.path.join(visual_dir, scene_id[batch_idx] + '_pred_labels.txt')
            np.savetxt(filename, final_data, fmt='%f %f %f %f %f %f %d')
